...
# 165 "mode0.c"
    current = collisionMap[((row + rowShift) * (mapWidth) + (col + width - 1 + colShift))];
    if (current < smallestSoFar) {
        smallestSoFar = current;
    }


    current = collisionMap[((row + height - 1 + rowShift) * (mapWidth) + (col + colShift))];
    if (current < smallestSoFar) {
        smallestSoFar = current;
    }
    ...
    current = collisionMap[((row + height - 1 + rowShift) * (mapWidth) + (col + width - 1 +colShift))];
    if (current < smallestSoFar) {
        smallestSoFar = current;
    }


    current = collisionMap[((row + (height/2) - 1 + rowShift) * (mapWidth) + (col + colShift))];
    if (current < smallestSoFar) {
        smallestSoFar = current;
    }


    current = collisionMap[((row + ((height/2) - 1) + rowShift) * (mapWidth) + (col + width - 1 +colShift))];
    if (current < smallestSoFar) {
        smallestSoFar = current;
    }
...
int footCollision(unsigned char *collisionMap, int mapWidth, int col, int row, int width, int height, int colShift, int rowShift) {
    ...
    current = collisionMap[((row + height - 1 + rowShift) * (mapWidth) + (col + colShift + ((width-1) /2)))];
    if (current < smallestSoFar) {
        smallestSoFar = current;
    }
