...
int collisionCheck(unsigned char *collisionMap, int mapWidth, int col, int row, int width, int height, int colShift, int rowShift) {
    ...
    // Top Right
    current = collisionMap[OFFSET(col + width - 1 + colShift, row + rowShift, mapWidth)];
    if (current < smallestSoFar) {
        smallestSoFar = current;
    }
    
    // Bottom Left
    current = collisionMap[OFFSET(col + colShift, row + height - 1 + rowShift, mapWidth)];
    if (current < smallestSoFar) {
        smallestSoFar = current;
    }
    ...
    // Bottom Right
    current = collisionMap[OFFSET(col + width - 1 +colShift, row + height - 1 + rowShift, mapWidth)];
    if (current < smallestSoFar) {
        smallestSoFar = current;
    }

    //Middle Left vert
    current = collisionMap[OFFSET(col + colShift, row + (height/2) - 1 + rowShift, mapWidth)];
    if (current < smallestSoFar) {
        smallestSoFar = current;
    }

    //Middle Right vertical
    current = collisionMap[OFFSET(col +  width - 1 +colShift, row + ((height/2) - 1) + rowShift, mapWidth)];
    if (current < smallestSoFar) {
        smallestSoFar = current;
    }
...
int footCollision(unsigned char *collisionMap, int mapWidth, int col, int row, int width, int height, int colShift, int rowShift) {
    ...
    // Bottom Middle
    current = collisionMap[OFFSET(col + colShift + ((width-1) /2), row + height - 1 + rowShift, mapWidth)];
    if (current < smallestSoFar) {
        smallestSoFar = current;
    }
